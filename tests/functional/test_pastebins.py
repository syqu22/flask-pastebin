...
def test_posting_empty_pastebin(test_client, init_database):
    ...
    response = test_client.post(
        "/", data=dict(title=None, content=None, syntax=None, expire_date=None, password=None))
...
def test_posting_pastebin_without_syntax(test_client, init_database):
    ...
    """
    response = test_client.post(
        "/", data=dict(title=None, content="test content", syntax=None, password=None, expire_date=None))
...
def test_accessing_pastebin_protected_with_password(test_client, init_database):
    ...
    response = test_client.post(
        "/" + pastebin.link, data=dict(password="password"), follow_redirects=True)
    assert response.status_code == 200
...
def test_delete_pastebin_by_creator(test_client, init_database, login_default_user):
    ...
    pastebin = Pastebin.query.filter_by(id=2).first()
    response = test_client.get(
        "/delete/" + pastebin.link, follow_redirects=True)
...
def test_pastebin_date_expiration(test_client, init_database):
    ...
    """
    response = test_client.post("/", data=dict(title="test title", content="test content",
                                syntax="text", password=None, expire_date="test"), follow_redirects=True)
